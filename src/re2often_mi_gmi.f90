...
submodule (re2often:re2often_mi) re2often_mi_gmi
...
contains
    ...
    contains
        real(c_double) function metric_fn(xhat, x) result(q)
            integer(c_int), intent(in) :: xhat
            integer(c_int), intent(in) :: x
    ...
    module function I_s_map_soft_direct(nm, s) result(I_s)
        ...
        integer(c_int) :: x, xtilde
        integer :: Ier

        real(c_double) :: gmi_s, sqrtN0
        ...
        I_s = I_s / ln2
    contains
        real(c_double) function q_map_soft_direct_prod(x, y) result (q)
            integer(c_int), intent(in) :: x
        ...
        real(c_double) function f_GH_map_expectation_ln_qs(u) result(f)
            ...
            do x = 0, nm%M-1
                f = f + nm%probabilities(x) * &
                    log0( q_map_soft_direct_prod(x, nm%constellation(x) + sqrtN0 * u) )
            end do
        ...
        function f_GH_map_expectation_qs(u) result(f)
            ...
            do x = 0, nm%M-1
                f = f + nm%probabilities(x) &
                    * q_map_soft_direct_prod(xtilde, nm%constellation(x) + sqrtN0*u)**gmi_s
            end do
    ...
    module function I_s_ml_hard_direct(nm, s) result (I_s)
        ...
        do xhat = 0, nm%M-1
            do x = 0, nm%M-1
                q_buff(x) = q_ml_hard_direct_prod(x, xhat)
            end do
    ...
    contains
        real(c_double) function q_ml_hard_direct_prod(x, xhat) result(q)
            integer(c_int), intent(in) :: x
            integer(c_int), intent(in) :: xhat
    ...
    contains
        real(c_double) function q_map_soft_reverse_prod(x, xhat, n) result(q)
            integer(c_int), intent(in) :: x
            integer(c_int), intent(in) :: xhat
        ...
        real(c_double) function f_N_GH_map_expectation_log_qs(n) result(f)
        ...
        real(c_double) function f_integrand(n) result(f)
            ...
            do x = 0, nm%M-1
                ...
                do xhat = 0, nm%M-1
                    ...
                    expected_q = expected_q + nm%delta_Fy(xhat)*(q**s_local)
                end do
                f_tmp = f_tmp*s_local - f_n_cond_x * log0(expected_q)
                f     = f + f_tmp*nm%probabilities(x)
            end do
