...
contains
  ...
  subroutine collect_suite(testsuite)
    ...
    testsuite = [&
         ...
         new_unittest("symbol_index_to_real", test_symbol_index_to_real)&
         ]
  end subroutine collect_suite

  
  subroutine test_bps_only(error)
    ...
    call check(error, pa%M, 16)
    if (allocated(error)) return

    call check(error, pa%B, 4)
    if (allocated(error)) return
    call check(error, pa%step, 2.0_wp, thr=0.001_wp)
    if (allocated(error)) return
    do i = 0, 15
       call check(error, pa%constellation(i), real(real(-15, wp) + real(i, wp)*2.0_wp, wp), thr=0.001_wp)
       if (allocated(error)) return
       call check(error, pa%probabilities(i), 0.0625_wp, thr=0.000001_wp)
       if (allocated(error)) return
    end do

    call check(error, pa%variance, 85.0_wp, thr=0.01_wp)
    if (allocated(error)) return
  end subroutine test_bps_only
